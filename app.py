...
app.secret_key = 'AgriGuide'
app.config['MONGODB_SETTINGS'] = {
     
    'host': 'mongodb://localhost:27017/swapnil',  
    'db': 'swapnil'  
...
model = pickle.load(open('C:/Users/spj55/Downloads/Capstone Project/AgriGuide/pickle/crops.pkl', 'rb'))
fert = pickle.load(open('C:/Users/spj55/Downloads/Capstone Project/AgriGuide/pickle/fertilizer1.pkl', 'rb'))
...
def predict():
    if request.method == "POST":
        ...
        cropdata= crop[prediction[0]]
        yield_value = cropdata["yield"]
        growing_season = str(cropdata["growing_season"])
        pests_and_diseases = cropdata["pests_and_diseases"][0]+" and  "+cropdata["pests_and_diseases"][1]+" etc"
        varieties = cropdata["varieties"][0]+"   " +cropdata["varieties"][1]+"   "+cropdata["varieties"][2]
        price_per_kg = cropdata["price_per_kg"]
    
    # Extracting prices for each variety if present
        prices_string=""
        prices = []
        for variety, price in price_per_kg.items():
            prices.append(f"{variety}: ₹  {price}")
            prices_string = ",   ".join(prices)
    
            # yield_value, growing_season, pests_and_diseases, varieties, prices
    
        return render_template("crop.html", prediction_text=prediction[0].capitalize(),
                               yield_value=yield_value,growing_season=growing_season,pests_and_diseases=pests_and_diseases,
                               varieties=varieties,prices=prices_string)
...
def predict2():
    ...
    if request.method == "POST":
        ...
        future_dates = [df1.index[-1] +
                        DateOffset(months=x)for x in range(0, 120)]
        ...
        fig = px.line(df2, x="Years(2018-2028)",
                      y="Predicted WPI(Wholescale Price Index)", title=f'Predicted Pice of {cropsgraph} in 2020-2028', markers=True)
...
disease_model_path = 'C:/Users/spj55/Downloads/Capstone Project/AgriGuide/pickle/plant_disease_model.pth'
